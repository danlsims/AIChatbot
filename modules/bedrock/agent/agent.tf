...
resource "null_resource" "agent_prepare" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      for i in {1..5}; do
        echo "Attempt $i: Preparing agent ${aws_bedrockagent_agent.bedrock_agent.id}"
        if aws bedrock-agent prepare-agent --agent-id ${aws_bedrockagent_agent.bedrock_agent.id}; then
          echo "Agent preparation successful"
          exit 0
        else
          echo "Agent preparation failed, waiting 30 seconds before retry..."
          sleep 30
        fi
      done
      echo "Agent preparation failed after 5 attempts, but continuing deployment..."
      exit 0
    EOT
  }
  depends_on = [
