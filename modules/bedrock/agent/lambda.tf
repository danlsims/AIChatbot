...
resource "aws_lambda_function" "bi_chatbot_container_lambda" {
  ...
  timeout                        = 300
  publish                        = true
  # Use local zip file for pure Terraform deployment
  filename         = var.lambda_zip_source_path
  source_code_hash = filebase64sha256(var.lambda_zip_source_path)
  package_type     = "Zip"
  memory_size      = 1024
  layers = [
...
# Provisioned concurrency set at 3 - TEMPORARILY DISABLED
# Uncomment after Lambda function is working properly
...
# Removed null_resource for S3-based Lambda code updates
# In pure Terraform approach, Lambda code updates are handled by source_code_hash
