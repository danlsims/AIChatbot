...
resource "aws_opensearchserverless_access_policy" "sample_kb_aoss_policy" {
  name = "aoss-kb-policy-${lower(var.app_name)}-${var.env_name}"
  type = "data"
  policy = jsonencode([
    {
      Rules = [
...
resource "aws_opensearchserverless_security_policy" "sample_kb_encryption_policy" {
  name = "aoss-encr-policy-${lower(var.app_name)}-${var.env_name}"
  type = "encryption"
  policy = jsonencode({
    Rules = [
      {
...
resource "aws_opensearchserverless_security_policy" "sample_kb_network_policy" {
  name = "aoss-nw-policy-${lower(var.app_name)}-${var.env_name}"
  type = "network"
  policy = jsonencode([
    {
      Rules = [
